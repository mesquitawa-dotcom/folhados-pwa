...
const { ref, set, get, update, remove, runTransaction } = require('firebase/database');
...
async function retryImutavel(db, path, reg) {
  const r = await runTransaction(ref(db, path), cur => cur == null ? reg : undefined, { applyLocally: false });
  const remoto = r.snapshot.val();
  return {
    committed: r.committed,
    igual: JSON.stringify(remoto) === JSON.stringify(reg),
    remoto,
  };
}
...
(async () => {
  ...
  try {
    ...
    const auditRetry = await retryImutavel(app, auditPath, audit);
    assert.strictEqual(auditRetry.committed, false, 'retry idempotente deve abortar sem escrever');
    assert.strictEqual(auditRetry.igual, true, 'retry idempotente deve reconhecer o remoto igual');
    console.log('OK   retry idempotente da auditoria é reconhecido sem nova escrita');
    const auditConflito = await retryImutavel(app, auditPath, { ...audit, motivo: 'conteúdo diferente' });
    assert.strictEqual(auditConflito.committed, false, 'conflito de auditoria não pode gravar');
    assert.strictEqual(auditConflito.igual, false, 'conflito deve ser detectável pelo cliente');
    ...
    const movRetry = await retryImutavel(app, movPath, mov);
    assert.strictEqual(movRetry.committed, false);
    assert.strictEqual(movRetry.igual, true);
    console.log('OK   retry idempotente do movimento é reconhecido sem nova escrita');
