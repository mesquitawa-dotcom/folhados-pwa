...
(async () => {
  ...
  try {
    ...
    await ok('partida pós-laminação pode nascer',set(ref(app,'fdo_v25/partidas/part_1'),{id:'part_1',qtd:40,situacao:'cortada'}));
    await ok('partida pode evoluir de estado',update(ref(app,'fdo_v25/partidas/part_1'),{situacao:'freezer_rapido',atualizadoTs:2}));
    await ok('forma pode nascer',set(ref(app,'fdo_v25/formas/forma_1'),{id:'forma_1',totalUnidades:16}));
    await ok('forma operacional pode ser atualizada',update(ref(app,'fdo_v25/formas/forma_1'),{atualizadoTs:2}));
    ...
    console.log('FIREBASE RULES v26.6 OK · novas coleções operacionais autorizadas · históricos append-only preservados');
