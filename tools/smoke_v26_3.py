...
try:
    cmd=[chrome,'--headless=new','--no-sandbox','--disable-gpu','--disable-background-networking','--virtual-time-budget=5000','--dump-dom','http://127.0.0.1:8765/_smoke_v26_3.html']
    ...
    if 'data-smoke-destino="ok"' not in dom or 'data-smoke-receita="ok"' not in dom:
        raise SystemExit('Funções críticas não ficaram disponíveis após o boot')
    print('SMOKE v26.3 OK · boot offline autorizado chegou à tela inicial')
finally:
    server.shutdown()
